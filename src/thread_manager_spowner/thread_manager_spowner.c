...
long mydev_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
		...
		case IOCTL_INSTALL_GROUP_T:
			...
			// copy_from_user(&gt, (group_t *)arg, sizeof(group_t));
			// printk("user sent dnosjsdjo %d\n", info.gt);
			ret = set_new_driver(info.gt);
			goto out;

		// case IOCTL_POPULATE:
		// 	memcpy(&info.payload, "From kernel", strlen("From kernel"));
		// 	int val =12;
		// 	memcpy(&info.gt, &val, sizeof(int));
